...
Version history:
  v1.1 Fix project directory issue (syntax fixed in ./_quarto.yml), add
       ...
       relative to original scales using a 2D tSNE embedding. Add 3d-t-SNE. 
...
We first check the existence of required files, and whether your course direcrtory is "in a sane state". That goes on under the hood of the first code chunk - you can inspect in in the `qmd` source, but it is hidden in the HTML document.

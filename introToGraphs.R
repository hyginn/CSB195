# tocID <- "introToGraphs.R"
#
# Purpose:  An introduction to biological graphs and Networks.
#
# Version:  2.0
#           (Derived from code developed for ABC learning units:
#            FND-MAT-Graphs_and_networks.R)
#
# Date:     2017-10  -  2023-11
# Author:   Boris Steipe (boris.steipe@utoronto.ca)
#
# Versions:
#           2.0    Reconceived as starter code for CSB 195 hackathon 2023
...
#TOC> --------------------------------------------------------------
#TOC>   1        REVIEW                                           55
#TOC>   2        EXPLORING DEGREE DISTRIBUTIONS                  201
#TOC>   2.1        Random graph                                  207
#TOC>   2.2        scale-free graph (Barabasi-Albert)            256
#TOC>   2.3        Random geometric graph                        322
#TOC>   3        A CLOSER LOOK AT THE igraph:: PACKAGE           446
#TOC>   3.1        Basics                                        449
#TOC>   3.2        Components                                    511
#TOC>   4        RANDOM GRAPHS AND GRAPH METRICS                 530
#TOC>   4.1        Diameter                                      567
#TOC>   4.2        Centralization scores                         580
#TOC>   5        GRAPH CLUSTERING                                637
#TOC>   6        BIOLOGICAL EXPLORATIONS                         681
#TOC> 
#TOC> ==========================================================================
...
# random graph og interacting genes. Here are some gene names, they are made-up
# names but real genes are named similarly.

geneNames <- c("Zlf4", "Qmc6", "Coj2", "Rzb7", "Dyj4", "Sgi7", "Rdm3",
               "Pwg1", "Qae8", "Cop3", "Mdm8", "Arb7", "Msg1", "Jpb5",
               "Tks4", "Dqa9", "Uwp1", "Nek6", "Ztq6","Ikj6")
...
# intersection of a row and column contains a value/TRUE if there is a
# connection or interaction, or some other relationship. In graph theory, we
# call this an edge. If there is no edge, the value in the cell is 0 or FALSE.
...
makeRandomAM <- function(nam, p = 0.1) {
  ...
  return(AM)
                                                 ...
                                                 # Set an RNG seed
set.seed(112358)                                 # for repeatable randomness.
( myRandAM <- makeRandomAM(geneNames, p=0.09) )  # Create the adjacency matrix.
...
# convert our nodes and edges into a computationally convenient form.
...
set.seed(112363)
myGxy <- igraph::layout_with_graphopt(myG, charge=0.0012)
...
plot(myG,
     ...
     vertex.size = 700 + (500 * igraph::degree(myG)),
...
# The function igraph::degree() gives the same values
igraph::degree(myG)
...
# =    2  EXPLORING DEGREE DISTRIBUTIONS  ======================================
...
# 200 node random geometric graph
N <- 200
...
plot(myG,
     ...
     xlim = c(min(myGxy[,1]) * 0.99, max(myGxy[,1]) * 1.01),
     ylim = c(min(myGxy[,2]) * 0.99, max(myGxy[,2]) * 1.01),
     vertex.color=hcl.colors(max(igraph::degree(myG)+1))[igraph::degree(myG)+1],
     vertex.size = 40 + (15 * igraph::degree(myG)),
     vertex.label.cex = 0.4 + (0.15 * igraph::degree(myG)),
     vertex.label = igraph::V(myG)$name,
     vertex.label.family = "sans")
...
plot(myGRG,
     ...
     xlim = c(min(rGAM$x) * 0.9, max(rGAM$x) * 1.1),
     ylim = c(min(rGAM$y) * 0.9, max(rGAM$y) * 1.1),
     vertex.color=hcl.colors(max(nodeBetw))[nodeBetw],
...
# That's all. Now let's do some explorations of biology.



# =    6  BIOLOGICAL EXPLORATIONS  =============================================


# (1) Access the STRING database.
#
# (2) Download human high-confidence interactions.
#
# (3) Find the genes with the highest betweeness centrality.
#
# (4) Figure out if the have something in common.
